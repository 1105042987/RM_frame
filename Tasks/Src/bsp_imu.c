...
void imu_yaw_normalize(void)
{
	if(imu.yaw<=imu.last_yaw)
		{
			if((imu.last_yaw-imu.yaw)>180)//陀螺仪上溢
				imu.now_yaw -= imu.last_yaw-imu.yaw - 360.0f;
			else//正常
				imu.now_yaw -= imu.last_yaw-imu.yaw;
		}
		else
		{
			if((imu.yaw-imu.last_yaw)>180)//陀螺仪下溢
				imu.now_yaw -= imu.last_yaw-imu.yaw + 360.0f;
			else//正常
				imu.now_yaw -= imu.last_yaw-imu.yaw;
		}
